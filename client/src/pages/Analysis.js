...
export default function Analysis({ transactions, setTransactions }) {
  ...
  const currentMonthToDateSum = currentMonthTransactions.reduce(
    (total, transaction) => {
  ...
  const currentYearToDateSum = currentYearTransactions.reduce(
    (total, transaction) => {
      return total + transaction.amount;
    },
    0
  ...
  const priorMonthTransactions = transactions.filter((transaction) => {
    const transactionDate = new Date(parseInt(transaction.date));
    return (
      transactionDate.getMonth() === priorMonth &&
      transactionDate.getFullYear() === currentYear
    );
  });

  // Sum transaction amounts for prior month to date
  const priorMonthToDateSum = priorMonthTransactions.reduce(
    (total, transaction) => {
      return total + transaction.amount;
    },
    0
  ...
  const priorYearToDateSum = priorYearTransactions.reduce(
    (total, transaction) => {
      return total + transaction.amount;
    },
    0
    ...
    datasets: [
      {
        ...
        data: [
          ...
          sumCategory.find((x) => x.category === "Restaurant/Fast-Food")
            ?.amount || 0,
          ...
          <div className="row">
            <div className="card card-chart ml-5">
              <div className="card-header card-chart-header">
                ...
                <h3 className="chart-title text-center text-light">
                  <span className="blue-text">Essential</span> vs{" "}
                  <span className="red-text">Non-Essential</span>
                </h3>
              </div>
